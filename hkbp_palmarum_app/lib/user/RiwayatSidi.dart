...
import 'package:hkbp_palmarum_app/user/DetailConfirmSidi.dart';
...
Future<List<dynamic>> fetchPemasukanList(String idJemaat) async {
  ...
  if (response.statusCode == 200) {
    ...
    throw Exception('Tidak Ada Request Sidi');
  }
}
...
class RiwayatSidi extends StatefulWidget {
  @override
  _RiwayatSidiState createState() => _RiwayatSidiState();
}

class _RiwayatSidiState extends State<RiwayatSidi> {
      ...
      body: FutureBuilder<List<dynamic>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(
              child: Text('Tidak Ada Request Sidi'),
            );
          } else {
              ...
              child: GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return GestureDetector(
                    onTap: () {
                      int pemasukanId = pemasukan['id_registrasi_sidi'];
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (context) => DetailConfirmSidi(idRegistrasiSidi: pemasukanId),
                        ),
                      );
                    },
                    child: Container(
